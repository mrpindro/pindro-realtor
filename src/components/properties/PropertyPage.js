...
const PropertyPage = () => {
    ...
    React.useEffect(() => {
        const getProp = async () => {
            try {
                const response = await axiosApi.get(`props/${id}`);
    ...
    const viewPrevImg = () => {
        let index = val <= prop.images.length - 1 && val > 0 ? val - 1 : val;
        setVal(index);
    ...
    const viewNextImg = () => {
        let index = val < prop.images.length - 1 ? val + 1 : val;
        setVal(index);
        const img = prop.images[index]
        setBigImg(img);
    }
    ...
    return (
        <main className='main-con buy-prop-main flex-col'>
            <div className='nav-back'>
                <Link to='/properties' className='nav-links'>← Back </Link>
            </div>
